...
接着说一下数组的情况，Vue 官网介绍由于js特性不能检测到以下变动的数组（其实是可以的，尤大因为性能原因舍弃了）：
...
我们刚才也分析到，对于 `Vue.set` 的实现，当 `target` 是数组的时候，也是通过 `target.splice(key, 1, val)` 来添加的，那么这里的 `splice` 为什么能让添加的对象变成响应式的呢。
