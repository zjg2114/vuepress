...
Vue 的组件对象支持了计算属性 `computed` 和侦听属性 `watch` ，接下里我们从源码实现的角度来分析它们两者有什么区别。
...
函数首先创建 `vm._computedWatchers` 为一个空对象，接着对 `computed` 对象做遍历，拿到计算属性的每一个 `userDef`，然后尝试获取这个 `userDef` 对应的 `getter` 函数，拿不到则在开发环境下报警告。接下来为每一个 `getter` 创建一个 `watcher`，这个 `watcher` 和渲染 `watcher` 不同，它是一个 `computed watcher`，最后对判断如果 `key` 不是 `vm` 的属性，则调用 `defineComputed(vm, key, userDef)`，否则判断计算属性对于的 `key` 是否已经被 `data` 或者 `prop` 所占用，如果是的话则在开发环境报相应的警告。
...
整个计算属性的初始化过程到此结束，接下来来我们来通过一个例子来分析 `computed watcher` 的实现，和`render watcher`又在哪。
