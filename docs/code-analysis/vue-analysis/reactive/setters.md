...
class Watcher {
  ...
  update () {
    ...
    if (this.computed) {
      // 计算属性有两种模式：lazy和active。默认lazy，当它被其他计算属性或组件的渲染函数依赖时会是active。
      if (this.dep.subs.length === 0) {
      //  在lazy模式下，dirty置为true， 实际计算是在this.evaluate（）中执行
        this.dirty = true
      } else {
        // 在active模式下，我们要主动执行计算，当值发生更改时通知订阅者。
        this.getAndInvoke(() => {
          this.dep.notify()
        })
      }
...
这里对于 `Watcher` 的不同状态，会执行不同的逻辑，一般组件数据更新的场景，会走到最后一个 `queueWatcher(this)` 的逻辑，`queueWatcher` 的定义在 `src/core/observer/scheduler.js` 中：
...
export function queueWatcher (watcher: Watcher) {
  ...
  if (has[id] == null) {
    ...
    if (!flushing) {
      ...
      // 如果队列已刷新，则根据id插入新的watcher
      // 如果已经队列中的id越过watcher的id，它会在下一刻立即运行。
...
这里有几个细节要注意一下，首先用 `has` 对象保证同一个 `Watcher` 只添加一次；接着对 `flushing` 的判断，else 部分的逻辑稍后我会讲；最后通过 `waiting` 保证对 `nextTick(flushSchedulerQueue)` 的调用逻辑只有一次， `nextTick` 可以理解它是在下一个 tick，也就是异步的去执行 `flushSchedulerQueue`。
...
function flushSchedulerQueue () {
  ...
  //当我们运行现有观察者时 不缓存长度，因为可能会推送更多的观察者
...
派发更新过程：当数据发生变化的时候，触发 setter 逻辑，把在依赖过程中订阅的的所有观察者，也就是 `watcher`，都触发它们的 `update` 过程，这个过程又利用了队列做了进一步优化，在 `nextTick` 后执行所有 `watcher` 的 `run`，最后执行它们的回调函数。`nextTick` 是 Vue 一个比较核心的api，我们来分析一下它的实现。