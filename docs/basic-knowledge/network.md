...
缓存处理：HTTP/1.1 引入了更多的缓存控制策略：Cache-Control、Etag/If-None-Match等
...
持久连接：HTTP/1.1 最大的变化就是引入了持久连接，字段Connection: keep-alive，即TCP连接默认不关闭，可以被多个请求复用,客户端可以在最后一个请求时，发送Connection: close，明确要求服务器关闭TCP连接

管道机制：HTTP/1.1中引入了管道机制（pipelining）,即在同一个TCP连接中，客户端可以同时发送多个请求。

HTTP/1.1 的持久连接和管道机制允许复用TCP连接，在一个TCP连接中，也可以同时发送多个请求，但服务器还是按照顺序来响应请求，如果队首的响应特别慢，会阻塞后面的请求，称为"队头阻塞"。
